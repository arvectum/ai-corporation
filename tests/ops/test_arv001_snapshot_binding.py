...
from scripts.arv001.full_pre_provider import _reconstruct_actual_batch_requests
...
def test_reconstruct_requests_selects_correct_run(tmp_path: Path) -> None:
    ...
    with Session() as session:
        # Run 1: Unrelated
        r1 = TenderAnalysisRun(id="wrong-run", registry_number="r1", status="completed", metadata_json="{}")
        session.add(r1)
    ...
    # Reconstruct
    requests = _reconstruct_actual_batch_requests(database, policy, tokenizer=tokenizer, descriptor=descriptor)
