...
class TestLockIntegration:
    def test_acquire_and_release(self, test_namespace, cleanup_keys):
        reset_redis_runtime()
        key = f"{test_namespace}:lock:test_acquire_release"
        token = acquire(key, ttl_seconds=10)
        assert token
        assert release(key, token) is True

    def test_second_acquire_rejected(self, test_namespace, cleanup_keys):
        reset_redis_runtime()
        key = f"{test_namespace}:lock:test_second_reject"
        token = acquire(key, ttl_seconds=10)
        assert token
        with pytest.raises(RedisAlreadyLockedError):
            acquire(key, ttl_seconds=10, wait_timeout_seconds=None)

    def test_foreign_token_release_rejected(self, test_namespace, cleanup_keys):
        reset_redis_runtime()
        key = f"{test_namespace}:lock:test_foreign_release"
        token1 = acquire(key, ttl_seconds=10)
        assert token1
        result = release(key, "fake-token")
        assert result is False

    def test_expired_owner_does_not_delete_new_lock(self, test_namespace, cleanup_keys):
        reset_redis_runtime()
        key = f"{test_namespace}:lock:test_expired_owner"
    ...
    def test_ttl_expiry(self, test_namespace, cleanup_keys):
        reset_redis_runtime()
        key = f"{test_namespace}:lock:test_ttl_expiry"
    ...
    def test_wait_timeout_raises(self, test_namespace, cleanup_keys):
        reset_redis_runtime()
        key = f"{test_namespace}:lock:test_wait_timeout"
    ...
    def test_concurrent_lock_one_winner(self, test_namespace, cleanup_keys):
        reset_redis_runtime()
        key = f"{test_namespace}:lock:concurrent"
