...
return {(allowed and "1" or "0"), tostring(current), tostring(remaining), tostring(retry_after), tostring(reset_at)}
"""
...
def check(key: str, limit: int | None = None, window_seconds: int | None = None) -> dict:
    ...
    try:
        ...
        raise RedisUnavailableError(f"Redis rate limit check unavailable: category={category}") from None
